    ...
    public class StartUp
    {
        static void Main(string[] args)
        {
            using (SqlConnection connection = new SqlConnection(Configuration.ConnectionString))
            {
                ...
                ExecuteNonQuerry(connection, createDatabaseSql);

                connection.ChangeDatabase("MinionsDB");

                ExecuteNonQuerry(connection, tableCountries);
                ExecuteNonQuerry(connection, tableTowns);
                ExecuteNonQuerry(connection, tableMinions);
                ExecuteNonQuerry(connection, tableEvilnessFactors);
                ExecuteNonQuerry(connection, tableVillains);
                ExecuteNonQuerry(connection, tableMinionsVillains);
                ...
                ExecuteNonQuerry(connection, insertCountries);
                ExecuteNonQuerry(connection, insertTowns);
                ExecuteNonQuerry(connection, insertMinions);
                ExecuteNonQuerry(connection, insertEvilnessFactors);
                ExecuteNonQuerry(connection, insertVillains);
                ExecuteNonQuerry(connection, insertMinionsVillains);

                connection.Close();
            }
        }

        private static void ExecuteNonQuerry(SqlConnection connection, string commandString)
        {
